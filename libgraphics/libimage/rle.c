...
*   UPDATES
*
*   $Id: rle.c,v 1.1.1.1 2004/05/03 03:56:49 michael Exp $
*   $Log: rle.c,v $
...
int RleEncodeFile(char *inFile, char *outFile)
{
    ...
    while ((currChar = fgetc(fpIn)) != EOF)
    {
        ...
        if (currChar == prevChar)
        {
            ...
            while ((fscanf(fpIn, "%d", (int *)&currChar)) != EOF)
            {
                fscanf(fpIn, "%c", (char *)&currState);
                if (currChar == prevChar && currState == prevState)
                {
                    count++;
                ...
                {
                    /* run ended */
                    fprintf(fpOut, "%d %d%c", count, currChar, currState);
                    //fputc(count, fpOut);
                    //fputc(currChar, fpOut);
                    prevChar = currChar;
                    prevState = currState;
